            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 bg-slate-50/50">
                {loading ? (
                    ...
                    messages.map((msg, index) => {
                        ...
                        // El primer mensaje se muestra como el "Contexto/Ticket Original"
                        if (index === 0) {
                            return (
                                <div key={msg.id} className="bg-white rounded-xl border border-indigo-100 shadow-sm p-4 mb-6 relative overflow-hidden">
                                    ...
                                    <div className="flex items-start justify-between mb-3 border-b border-slate-100 pb-3">
                                        <div>
                                            ...
                                            <h2 className="text-base font-bold text-slate-800 leading-tight">
                                                {thread?.subject || "Asunto no especificado"}
                                            </h2>
                                            <p className="text-xs text-slate-500 mt-1">
                                                Iniciado por <span className="font-semibold text-slate-700">{senderName}</span>
                                                {msg.sender?.role && ` (${msg.sender.role})`} • {new Date(msg.created_at).toLocaleString("es-CL", { dateStyle: "medium", timeStyle: "short" })}
                                            </p>
                                        </div>
                                    </div>
                        ...
                        return (
                            <div key={msg.id} className={cn("flex flex-col gap-0.5", isOwn ? "items-end" : "items-start")}>
                                ...
                                <div className={cn(
                                    ...
                                    isOwn ? "bg-primary text-white rounded-tr-sm" : "bg-white border border-slate-200 text-slate-800 rounded-tl-sm"
                                )}>
                                    {msg.content}
                                </div>
