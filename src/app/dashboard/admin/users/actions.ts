        ...
        const usersWithEmail = await Promise.all(
            (users || []).map(async (userProfile) => {
                try {
                    ...
                    console.error(`Error fetching email for user ${userProfile.id}:`, err)
                    return {
                        ...userProfile,
                        email: ''
                    }
...
export async function getUsersByRole(role: string) {
    try {
        ...
        return { error: error.message || 'Error al obtener usuarios por rol' }
    }
}
    ...
    try {
        ...
        if (data.role === 'student' && data.course_id) {
            ...
            if (enrollError) {
                ...
                // We don't rollback user creation here, but maybe we should warn?
            }
        }
