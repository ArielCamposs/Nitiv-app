...
import { Home, Bell, Users, FileText, Calendar, BookOpen, Lightbulb, LogOut } from 'lucide-react'
            ...
            <nav className="fixed bottom-0 left-0 right-0 bg-white border-t-2 border-gray-200 shadow-2xl z-50 md:hidden overflow-x-auto">
                <div className="flex items-center h-20 px-2">
                    {NAV_ITEMS.map((item) => {
                        ...
                        return (
                            <Link
                                ...
                                href={item.href}
                                className={`
                                    flex flex-col items-center justify-center gap-1 flex-shrink-0 w-[70px] py-2
                    ...
                    <button
                        ...
                        className="flex flex-col items-center justify-center gap-1 flex-shrink-0 w-[70px] py-2 transition-all duration-200 opacity-70 hover:opacity-100"
                    >
                        <div className="w-10 h-10 rounded-2xl flex items-center justify-center bg-red-100 hover:bg-gradient-to-br hover:from-red-400 hover:to-red-600 transition-all duration-200">
                            <LogOut size={20} className="text-red-600 hover:text-white transition-colors" strokeWidth={2.5} />
                        </div>
            ...
            <aside className="hidden md:flex fixed left-0 top-0 bottom-0 w-32 bg-white border-r-2 border-gray-200 shadow-xl z-40 flex-col items-center py-4 gap-1 overflow-x-hidden">
                ...
                >
                    <div className="w-11 h-11 rounded-xl flex items-center justify-center bg-red-100 hover:bg-gradient-to-br hover:from-red-400 hover:to-red-600 transition-all duration-200">
                        <LogOut size={22} className="text-red-600 hover:text-white transition-colors" strokeWidth={2.5} />
                    </div>
                    <span className="text-[11px] font-bold text-red-600 text-center leading-tight">Cerrar<br />sesión</span>
                </button>
            </aside>
        </>
    )
