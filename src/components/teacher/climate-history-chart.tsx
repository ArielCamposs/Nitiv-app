...
function hexToRgba(hex: string, alpha: number): string {
    ...
    if (Number.isNaN(r) || Number.isNaN(g) || Number.isNaN(b)) return `rgba(148, 163, 184, ${alpha})`
    return `rgba(${r},${g},${b},${alpha})`
}

export function ClimateHistoryChart({ courses, historyLogs, showFilters: _showFilters, teachers }: Props) {
    void _showFilters
    ...
    useEffect(() => {
        if (courses.length > 0 && selectedCourse === "all") {
            ...
            const firstId =
                firstWithLogs?.id || firstWithLogs?.course_id || courses[0]?.id || courses[0]?.course_id
            if (firstId) setSelectedCourse(firstId)
        }
    }, [courses, historyLogs, selectedCourse])
    ...
    const stats = useMemo(() => {
        ...
        const mostFrequent = entries.length > 0 ? entries.sort((a, b) => b[1] - a[1])[0][0] : null

        const teacherMap: Record<string, number> = {}
        weekLogs.forEach((l) => {
            if (l.teacher_id) teacherMap[l.teacher_id] = (teacherMap[l.teacher_id] ?? 0) + 1
        })
    ...
    const legendKeys = CLASSROOM_CLIMATES
                ...
                <div className="flex items-start justify-between gap-6 flex-wrap">
                    <div className="space-y-1">
                        ...
                        <p className="text-sm text-slate-500 max-w-md">
                            Color según el clima registrado por bloque y día. ({historyLogs.length} registros totales)
                        </p>
                    </div>
                        ...
                        <div className="lg:col-span-6 grid grid-cols-2 sm:grid-cols-3 gap-3">
                            ...
                            <div className="p-3 rounded-2xl bg-linear-to-br from-indigo-50 to-white border border-indigo-100 shadow-xs text-indigo-900 col-span-2 sm:col-span-1">
                                ...
                                <p className="text-xs font-bold truncate">
                                    {stats?.mostFrequent ? climateLabel(stats.mostFrequent) : "N/A"}
                                </p>
                            </div>
                        </div>

                        <div className="lg:col-span-3 p-3 rounded-2xl bg-slate-50 border border-slate-100 flex flex-col justify-center max-h-44 overflow-y-auto">
                                ...
                                {legendKeys.map((key) => (
                                    <div key={key} className="flex items-center gap-1 min-w-0">
                                        <div
                                            ...
                                            style={{ backgroundColor: climateColor(key) }}
                                        />
                            ...
                            <div className="flex items-start gap-2">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <p className="text-[9px] text-amber-800 leading-tight">
                                        Registros de todos los docentes por bloque (1–12). Incluye modelo histórico (explosiva,
                                        inquieta, etc.) si existiera.
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                {selectedCourse === "all" ? (
                    <div className="flex flex-col items-center justify-center h-80 bg-slate-50 rounded-2xl border border-dashed border-slate-200 text-slate-400 gap-3">
                        ...
                        <div className="text-center">
                            ...
                            <p className="text-xs">
                                Para visualizar el calendario semanal debes filtrar por un curso específico.
                            </p>
                        </div>
                    </div>
                ) : (
                    <div className="overflow-x-auto rounded-2xl border border-slate-200 shadow-sm max-w-3xl mx-auto w-full">
                        <div className="min-w-[500px] bg-white">
                            <div className="grid grid-cols-[60px_repeat(5,1fr)] bg-slate-50 border-b border-slate-200 sm:grid-cols-[80px_repeat(5,1fr)]">
                                ...
                                {DAYS_OF_WEEK.map((day) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <p className="text-[11px] font-black text-slate-800 uppercase tracking-wider sm:text-xs">
                                                {day.name}
                                            </p>
                            ...
                            {Array.from({ length: 12 }, (_, i) => i + 1).map((blockNum) => (
                                <div
                                    key={blockNum}
                                    className="grid grid-cols-[60px_repeat(5,1fr)] border-b border-slate-100 last:border-0 hover:bg-slate-50/50 transition-colors sm:grid-cols-[80px_repeat(5,1fr)]"
                                >
                                    <div className="p-2 flex flex-col items-center justify-center border-r border-slate-200 bg-slate-50/30">
                                        ...
                                        <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter hidden sm:inline">
                                            Bloque
                                        </span>
                                    </div>
                                    ...
                                    {DAYS_OF_WEEK.map((day) => {
                                        ...
                                        const teacherName =
                                            teachers?.find((t) => t.id === log?.teacher_id)?.name ?? "Docente"
                                        const col = climateColor(log?.energy_level)

                                        return (
                                            <div
                                                key={day.key}
                                                className="border-r border-slate-100 last:border-r-0 min-h-[56px]"
                                            >
                                                {log ? (() => {
                                                    const isLight = log.energy_level === "no_aplica" || log.energy_level === "parcial"
                                                    const textColorClass = isLight ? "text-slate-800" : "text-white"
                                                    const iconColorClass = isLight ? "text-slate-700" : "text-white/90"
                                                    const pillBgClass = isLight ? "bg-black/10 border-black/10" : "bg-white/20 border-white/20"

                                                    return (
                                                        <div
                                                            className={`w-full h-full min-h-[56px] flex flex-col items-center justify-center gap-1 animate-in fade-in duration-500 ${textColorClass}`}
                                                            style={{ backgroundColor: col }}
                                                        >
                                                            <div className="flex items-center justify-center gap-1 w-full px-1">
                                                                <User className={`w-2.5 h-2.5 shrink-0 ${iconColorClass}`} />
                                                                <span
                                                                ...
                                                                {log.session_time && (
                                                                    <span className={`text-[6px] font-black border px-1 py-0.5 rounded-sm shrink-0 leading-none ${pillBgClass}`}>
                                                                        {log.session_time === "morning" ? "M" : "T"}
                                                                    </span>
                                                                )}
                                                            </div>
                                                            <div
                                                                className="text-[8px] font-black uppercase tracking-wider w-full text-center truncate px-1"
                                                                title={climateDefinition(log.energy_level)?.desc ?? ""}
                                                            >
                                                                {climateLabel(log.energy_level) === "Extraordinario" ? "Extra." : climateLabel(log.energy_level)}
                                                            </div>
                                                        </div>
                                                    )
                                                })() : (
                                                    <div className="w-full h-full min-h-[56px] bg-slate-50 flex items-center justify-center">
